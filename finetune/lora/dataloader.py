...
class HESignature(Dataset):
    ...
    def __init__(
        ...
        if not self.transform:
            self.transform = T.Compose(
                [T.Resize(args.img_size), T.CenterCrop(args.img_size), T.ToTensor()]
            )
        ...
        self.input_files, self.target_dict, self.stratif_dict = self._make_db()
    ...
    def _make_db(self):
        ...
        and values :
            ...
            * stratif_dict : their stratif values (present in the input_table).
        :return [files, target_dict, stratif_dict]
        """
        target_dict = dict()  # Key = path to the file, value = target
        stratif_dict = dict()

        table = self.input_table
        self.img_to_fold = dict(zip(table["img_path"], table["test"]))
        self.img_to_stratif = dict(zip(table["img_path"], table["stratif"]))
        ...
        basenames = table["img_path"].values
        files_filtered = []
        # add tqdm
        for name in basenames:
            ...
            stratif_dict[filepath] = self.img_to_stratif[name]
...
class HEImage(Dataset):
    ...
    def __init__(
        ...
        if args.input_path is not None:
            self.input_table = pd.read_csv(args.input_path)
        self.input_files, self.stratif_dict = self._make_db()
    ...
    def _make_db(self):
        ...
        if self.input_table is not None:
            ...
            self.img_to_stratif = dict(
                zip(self.input_table["img_path"], self.input_table["stratif"])
            )
            ...
            for name in basenames:
                ...
                files_filtered.append(filepath)
                stratif_dict[filepath] = self.img_to_stratif[name]
            return files_filtered, stratif_dict
        else:
