...
class BaseVisualizer(ABC):
    ...
    def _get_image(self, path_raw, info):
        ...
        image = slide.read_region(
            location=(param["x"], param["y"]), level=param["level"], size=(448, 448)
...
class TileSeeker(BaseVisualizer):
    ...
    def forward(self, wsi_ID):
        ...
        if self.store:
            self.store_best(
                ...
                max_per_slides=self.max_per_slides,
            )
        return self
    ...
    def store_best(
        self, inp, out, info, selection_att, min_prob=False, max_per_slides=None
    ):
        """store_best.
        ...
        for s in range(out.shape[0]):
            ...
            for o in range(len(self.label_encoder.classes_)):
                ...
                if (len(self.store_score[o]) < self.n_best) or (
                    sgn * out[s, o] >= sgn * self.store_score[o][0]
                ):
        ...
        for o, i in enumerate(self.label_encoder.classes_):
            ...
            self.store_preclassif[o] += list(
                np.array(tmp_preclassifstore[o])[selection]
            )
        ...
        for o in range(len(self.label_encoder.classes_)):
            ...
            self.store_attention[o] = list(
                np.array(self.store_attention[o])[indices_best]
            )
            self.store_preclassif[o] = list(
                np.array(self.store_preclassif[o])[indices_best]
            )
    ...
    def extract_images(self):
        for o, i in enumerate(self.label_encoder.classes_):
            ...
            self.store_image[i] = [
                self._get_image(self.path_raw, x).convert("RGB")
                for x in self.store_info[o]
            ]
        return self
...
class ConsensusTileSeeker(TileSeeker):
    def __init__(
        ...
        super(ConsensusTileSeeker, self).__init__(
            model[0], n_best, min_prob, max_per_slides, path_emb, att_thres, False
        )
        tile_seekers = []
        for m in model:
            ts = TileSeeker(
                m, n_best, min_prob, max_per_slides, path_emb, att_thres, False
            )
...
class HeatmapMaker(BaseVisualizer):
    ...
    def make_heatmap(self, wsi_ID, out, downsample_factor=16):
        ...
        for class_idx in range(logits.shape[1]):
            ...
            heatmaps_per_class[self.label_encoder.classes_[class_idx]], background = (
                self.create_heatmap(infomat, heatmap_scores)
            )

        max_over_classes = np.max(
            np.stack([heatmap for heatmap in heatmaps_per_class.values()])
        )
        min_over_classes = np.min(
            np.stack([heatmap for heatmap in heatmaps_per_class.values()])
        )
    ...
    def overlay_heatmap_on_thumbnail(
        ...
        thumbnail = wsi_image.read_region(
            (0, 0), level, wsi_image.level_dimensions[level]
        )
        ...
        heatmap_resized = zoom(
            heatmap_normalized, (scale_factor_y, scale_factor_x), order=0
        )  # order = 0 if you want to keep the original tile geometry
        ...
        neutral_background = Image.new(
            "RGB", colored_heatmap_pil.size, color=tuple(neutral_color)
        )
        mask = Image.fromarray(
            (~background_resized).astype(np.uint8) * 255
        )  # Invert mask for correct transparency
        combined_heatmap = Image.composite(
            colored_heatmap_pil, neutral_background, mask
        )
        thumbnail_w_background = Image.composite(thumbnail, neutral_background, mask)
        combined_heatmap = Image.blend(
            thumbnail_w_background, combined_heatmap, alpha=0.65
        )
