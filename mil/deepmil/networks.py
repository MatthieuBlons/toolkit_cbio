...
class MultiHeadAttention(Module):
    ...
    def forward(self, x):
        ...
        x = (
            torch.matmul(x, self.atn_layer_2_weights) + self.atn_layer_2_bias
        )  # 4 scores.
...
class MHMC_layers(Module):
    ...
    def __init__(self, args):
        ...
        classifier.append(
            LinearBatchNorm(
                int(args.feature_depth * self.num_heads),
                width_fe,
                args.dropout,
                args.constant_size,
            )
        ...
        for i in range(self.n_layers_classif):
            classifier.append(
                LinearBatchNorm(width_fe, width_fe, args.dropout, args.constant_size)
            )
    ...
    def __init__(self, args):
        ...
        if self.args.instance_transf:
            self.instance_transf = Sequential(
                LinearBatchNorm(
                    int(args.feature_depth), 256, args.dropout, args.constant_size
                ),
        ...
        classifier.append(
            LinearBatchNorm(
                int(dim_classif), width_fe, self.args.dropout, self.args.constant_size
            )
        )
        for i in range(self.n_layers_classif):
            classifier.append(
                LinearBatchNorm(
                    width_fe, width_fe, self.args.dropout, self.args.constant_size
                )
            )
        classifier.append(Linear(width_fe, self.num_class))
        classifier.append(LogSoftmax(-1))
...
class PoolingFunction(Module):
    def __init__(self, args):
        ...
        if self.pooling in ["ilse", "max"]:
            ...
            self.attention = Sequential(MultiHeadAttention(args))
    ...
    def forward(self, x):
        if self.pooling == "ilse":
            ...
            topk = torch.gather(
                x, 1, torch.cat([topk.unsqueeze(-1)] * self.args.feature_depth, axis=-1)
            )
            lowk = torch.gather(
                x, 1, torch.cat([lowk.unsqueeze(-1)] * self.args.feature_depth, axis=-1)
            )
...
class model1S(Module):
    ...
    def __init__(self, args):
        ...
        self.continuous_clusters = Sequential(
            Conv1d(
                in_channels=args.feature_depth, out_channels=n_clusters, kernel_size=1
            ),
        ...
        self.classifier = Sequential(
            Linear(in_features=n_clusters, out_features=hidden_fcn),  # Hidden_fc
            norm_layer(hidden_fcn),
            ReLU(),
            Dropout(p=args.dropout),
            Linear(in_features=hidden_fcn, out_features=1),
            Sigmoid(),
        )
...
class Conan(Module):
    ...
    def __init__(self, args):
        ...
        self.continuous_clusters = Sequential(
            Conv1d_bn(
                in_channels=args.feature_depth,
        ...
        self.weights = Sequential(
            Conv1d(in_channels=self.hidden1, out_channels=1, kernel_size=1), ReLU()
        )
        self.classifier = Sequential(
            Dense_bn(
                in_channels=(self.hidden1 + 1) * 2 * self.k + self.hidden1,
                out_channels=self.hidden_fcn,
                dropout=args.dropout,
                use_bn=use_bn,
            ),
            Dense_bn(
                in_channels=self.hidden_fcn,
                out_channels=self.hidden_fcn,
                dropout=args.dropout,
                use_bn=use_bn,
            ),
            Linear(in_features=self.hidden_fcn, out_features=args.num_class),
            LogSoftmax(-1),
        )
    ...
    def forward(self, x):
        ...
        ## Aggregation
        selection = torch.cat(
            (indices[:, :, : self.k], indices[:, :, -self.k :]), axis=-1
        )
        ...
        out = torch.cat(
            (scores.flatten(1, -1), avg.flatten(1, -1), out.flatten(1, -1)), axis=-1
        )
        out = self.classifier(out)
        return out
...
class SelfAttentionMIL(Module):
    ...
    def __init__(self, args):
        ...
        self.wsi_score = Sequential(Linear(self.L, 1))
