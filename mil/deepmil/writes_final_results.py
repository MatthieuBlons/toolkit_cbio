...
def soft_voting(final_res, table):
    ...
    for test in final_res.keys():
        ...
        voting_scores = scores.mean(0)  # mean prediction on n_ensemble best models
        ...
        preds += [
            res[0]["label_encoder"].inverse_transform([x])[0] for x in elected_pred
        ]
        ids += indices
        tests += [test] * len(indices)

        metrics_dict = compute_metrics(
            gt, elected_pred, voting_scores, num_class=voting_scores.shape[-1]
        )
...
def main(raw_args=None):
    ...
    parser.add_argument(
        ...
        default=".",
        type=str,
        help="path to the folder where the best models are stored.",
    )
    args = parser.parse_args(raw_args)

    models_path = glob(
        os.path.join(args.path, "model_best_test_*_repeat_*.pt.tar"), recursive=True
    )
    model = load_model(models_path[0], "cpu")
    table = pd.read_csv(model.args.target_path)
